...
class report_account_receivable(osv.osv):
    ...
    _name = 'account.report_account_receivable'
    _description = 'Account Receivable Report'
    _auto = False
    _columns = {
                            'name' : fields.char(string='# Invoice', size=100, readonly=True),
                            'partner_code' : fields.char(string='Customer Code', size=30, readonly=True),
                            'partner_id' : fields.many2one(string='Customer', obj='res.partner', readonly=True),
    ...
    def init(self, cr):
        tools.drop_view_if_exists(cr, 'account_report_account_receivable')
        strSQL =    """
                            CREATE OR REPLACE view account_report_account_receivable AS (
                                SELECT    
                                                    A.id AS id,
                                                    A.number AS name,
